""" Implementation of U-Net and related blocks """
import math
...
class TimeEmbedding(nn.Module):
    """
    Embeddings for iteration steps
    """
...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        time_channels: the number channels in the time step embeddings
...
class AttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # TODO: torch.scaled_dot_product_attention from PyTorch 2.0!!!
...
class DownBlock(nn.Module):
    ...
    def __init__(
        ...
        out_channels: the number of input channels
        time_channels: the number channels in the time step embeddings
        has_attn: whether to use attention
        """
        ...
        # If we have attention, we add it, otherwise we use an identity layer
        if has_attn:
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        """
        Initialize the layer
        image_channels: the number of channels in the image. 3 for RGB.
        n_channels: number of channels in the initial feature map that we transform the image into
        ch_mults: the list of channel numbers at each resolution. The number of channels is ch_mults[i] * n_channels
        is_attn: a list of booleans that indicate whether to use attention at each resolution
        n_blocks: the number of UpDownBlocks at each resolution
        """
